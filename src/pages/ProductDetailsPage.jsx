  ...
  return (
    <div className="container" style={{ padding: '40px 0' }}>
      <div style={{ display: 'flex', gap: 40, background: '#fff', padding: 32, borderRadius: 16 }}>
        <img src={product.photo} alt={product.name} style={{ width: 300, height: 300, objectFit: 'contain' }} />
        <div>
          <h2>{product.name}</h2>
          <p style={{ color: 'var(--text-secondary)' }}>Brand: {product.brand}</p>
          <p style={{ fontSize: 24, fontWeight: 'bold', margin: '16px 0' }}>${product.price}</p>
          ...
          <div style={{ display: 'flex', gap: 12, alignItems: 'center' }}>
            ...
            <button
              ...
              style={{ background: 'var(--primary-green)', color: '#fff', padding: '10px 20px', borderRadius: 8 }}
            >
              Add to cart
            </button>
          </div>
      ...
      <div style={{ marginTop: 32, background: '#fff', padding: 24, borderRadius: 16 }}>
        <div style={{ display: 'flex', gap: 16, borderBottom: '1px solid #eee', paddingBottom: 12 }}>
          <button onClick={() => setTab('description')} style={{ fontWeight: tab === 'description' ? 'bold' : 'normal' }}>
            Description
          </button>
